...
from ._spu import (
    ...
    SHM_NAME_TEMP, SHM_TEMP_SIZE,
    SHM_SNAP_HDR, ALS_REPORT_LEN,
    ACCEL_SCALE, GYRO_SCALE,
    IMU_DECIMATION,
)
from .orientation import MahonyAHRS
from . import _spu_hid

_ALS_LUX_OFF = 40
_ALS_CH_OFFSETS = [20, 24, 28, 32]
_NATIVE_RATE_HZ = 800
_STARTUP_PROBE_S = 2.0

# worker backends: 'corehid' uses the public CoreHID framework via the spu_hid
# helper, 'ctypes' is the original hand-rolled IOKit path. both write the same
# shared-memory layout, so callers cannot tell them apart.
_WORKERS = {
    'corehid': _spu_hid.hid_worker,
    'ctypes': sensor_worker,
}
...
class IMU:
    ...
    Runs unprivileged where the SPU HID driver allows it (macOS 27), and
    raises PermissionError if a release that requires root denies the HID
    open. Spawns a background worker thread
    ...
    def __init__(self, accel: bool = True, gyro: bool = True,
                 als: bool = False, lid: bool = False, temp: bool = False,
    ...
    def start(self) -> None:
        """Start the sensor worker thread.

        Tries to open the HID devices unprivileged first. macOS releases
        whose SPU HID driver allows that (verified on macOS 27) stream
        normally; on releases that gate IOKit HID access behind root the
        worker opens nothing and no reports arrive, in which case this
        raises PermissionError suggesting sudo.
        """
        if self._started:
            return
        if self._mock:
        ...
        if self._want_temp:
            self._shm_temp = self._create_shm(SHM_NAME_TEMP, SHM_TEMP_SIZE)
            kwargs['temp_shm_name'] = SHM_NAME_TEMP

        self._shutdown.clear()
        self._backend = self._pick_backend()
        self._worker = threading.Thread(
            target=_WORKERS[self._backend],
            args=(SHM_NAME, 0),
            kwargs=kwargs,
            daemon=True,
        )
        ...
        if not self._wait_for_first_report():
            running_as_root = os.geteuid() == 0
            self.stop()
            extra = ("" if running_as_root else
                     " -- this macOS version may require root for IOKit "
                     "HID access, try running with sudo")
            raise PermissionError(
                f"no sensor reports arrived within {_STARTUP_PROBE_S:.1f}s "
                f"of starting the worker{extra}")
    ...
    def read_temperature(self) -> Optional[float]:
        """Latest ambient temperature in degrees C, or None if unchanged.

        This is the light sensor's companion thermometer, sitting inside the
        machine, so it reads a degree or two above room air.
        """
        if not self._shm_temp:
            return None
        data, self._last_temp_count = shm_snap_read(
            self._shm_temp.buf, self._last_temp_count, 4)
        if data is None:
            return None
        return struct.unpack('<f', data)[0]
    ...
    def _pick_backend(self) -> str:
        ...
        MACIMU_BACKEND forces 'corehid' or 'ctypes'. Otherwise prefer CoreHID
        when its helper is present or buildable (it needs no privileged
        IORegistry writes), and fall back to the ctypes path.
        """
        forced = os.environ.get('MACIMU_BACKEND', '').strip().lower()
        if forced in _WORKERS:
            return forced
        if forced:
            raise ValueError(
                f"MACIMU_BACKEND={forced!r} not recognised; "
                f"use one of {sorted(_WORKERS)}")
        try:
            if _spu_hid.available():
                return 'corehid'
        except Exception:
            pass
        return 'ctypes'
    ...
    def backend(self) -> Optional[str]:
        """Which worker backend is in use, or None before start()."""
        return self._backend

    def _wait_for_first_report(self, timeout: float = _STARTUP_PROBE_S) -> bool:
        """Wait until the worker publishes at least one sample.

        Peeks the ring-buffer counter in the shm header rather than calling
        read_accel(), so the caller still gets every sample. Returns False
        on timeout, which in practice means the HID open was denied.
        """
        if not self._shm_accel:
            return False
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            total, = struct.unpack_from('<Q', self._shm_accel.buf, 4)
            if total:
                return True
            time.sleep(0.02)
        return False
    ...
    def _cleanup_stale_shm(self):
        ...
        for name in names:
            try:
                ...
                raise PermissionError(
                    f"shared memory segment {name!r} is owned by another user "
                    "(left over from a previous sudo run) and cannot be "
                    "removed without root -- run once as root to clear it, "
                    "or reboot") from e
