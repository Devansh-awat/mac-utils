...
USAGE_ALS_TEMP = 5              # ambient light sensor's thermometer
...
TEMP_REPORT_LEN = 14            # als-temp report bytes
TEMP_OFF = 2                    # q16 fixed point, /65536 = degC
...
def temp_from_report(payload):
    """als-temp report -> degrees C. q16 fixed point at offset 2."""
    raw, = struct.unpack_from('<I', payload, TEMP_OFF)
    return raw / 65536.0
...
def sensor_worker(shm_name, restart_count, gyro_shm_name=None,
    ...
    if temp_buf is not None:
        def on_temp_report(ctx, result, sender, rtype, rid, rpt, length, ts):
            if _shutting_down[0]:
                return
            try:
                if length == TEMP_REPORT_LEN:
                    celsius = temp_from_report(bytes(rpt[:TEMP_REPORT_LEN]))
                    struct.pack_into('<f', temp_buf, SHM_SNAP_HDR, celsius)
                    cnt, = struct.unpack_from('<I', temp_buf, 0)
                    struct.pack_into('<I', temp_buf, 0, cnt + 1)
            except Exception:
                pass

        temp_cb_ref = _TS_REPORT_CB(on_temp_report)
    ...
    while True:
        ...
        if (up, u) == (PAGE_VENDOR, USAGE_ACCEL):
            ...
            cb = lid_cb_ref
        elif (up, u) == (PAGE_VENDOR, USAGE_ALS_TEMP) and temp_cb_ref is not None:
            cb = temp_cb_ref
